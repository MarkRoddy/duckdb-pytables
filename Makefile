...
debug:
	mkdir -p  build/debug && \
	cmake $(GENERATOR) $(FORCE_COLOR) $(EXTENSION_FLAGS) ${CLIENT_FLAGS} ${CIFLAGS} -DENABLE_SANITIZER=TRUE -DFORCE_ASSERT=TRUE -DEXTENSION_STATIC_BUILD=1 -DCMAKE_BUILD_TYPE=Debug ${BUILD_FLAGS} -S ./duckdb/ -B build/debug && \
	cmake --build build/debug --config Debug

release:
	echo "CIFLAGS: ${CIFLAGS}" && \
	echo "Python3_ROOT_DIR: ${Python3_ROOT_DIR}" && \
	echo "LD_LIBRARY_PATH: ${LD_LIBRARY_PATH}" && \
	find ${Python3_ROOT_DIR} && \

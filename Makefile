...
ifeq ($(UNAME_S),Darwin)
	OSX_TARGET_ARCH := $(if $(OSX_TARGET_ARCH),$(OSX_TARGET_ARCH),arm64)
	BUILD_FLAGS:=$(BUILD_FLAGS) -DOPENSSL_ROOT_DIR=/usr/local/opt/openssl
	BUILD_FLAGS:=$(BUILD_FLAGS) -DCMAKE_OSX_ARCHITECTURES=$(OSX_TARGET_ARCH)
	BUILD_FLAGS:=$(BUILD_FLAGS) -DCMAKE_C_COMPILER_TARGET=$(OSX_TARGET_ARCH)-apple-macos11
	BUILD_FLAGS:=$(BUILD_FLAGS) -DCMAKE_CXX_COMPILER_TARGET=$(OSX_TARGET_ARCH)-apple-macos11 ..
endif
